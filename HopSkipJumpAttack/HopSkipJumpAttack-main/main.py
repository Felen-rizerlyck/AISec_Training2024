...
from hsja import hsja, clip_image
import numpy as np
...
def attack(args):
	...
	for i, sample in enumerate(x_test[:args.num_samples]):
		...
		#记录样本的标签
		image_s = clip_image(sample, 0, 1)
		image_p = clip_image(perturbed, 0, 1)
		prediction1 = model.predict(image_s)
		prediction2 = model.predict(image_p)
		original_label = np.argmax(prediction1)
		attacked_label = np.argmax(prediction2)
		image = (image * 255).astype(np.uint8)
		image_pil = Image.fromarray(image)

		#在图片名称中体现标签变化
		image_pil.save('{}/figs/{}-{}-{}.jpg'.format(data_model,
													 original_label, attacked_label, i))

		#下为正常使用PIL中函数储存图片的函数
		#image_pil.save('{}/figs/{}-{}-{}.jpg'.format(data_model,
													 #args.attack_type, args.constraint, i))

		#下为原始代码，因imsave函数已弃用故进行修改
		# scipy.io.imsave('{}/figs/{}-{}-{}.jpg'.format(data_model,
